...
export default function GenresSelector({ badge, onClick }) {
	const renderBadge = () => {
		...
		return (
			<span className='dark:bg-dark-subtle bg-light-subtle text-white absolute top-0 right-0 translate-x-2 -translate-y-1 text-xs w-5 h-5 rounded-full flex justify items-center'>
				{badge <= 9 ? badge : '9+'}
			</span>
		)
	}
